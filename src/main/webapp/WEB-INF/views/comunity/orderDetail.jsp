      ...
      <div class="clm_order_detail">
        ...
        <div class="clm_acdo_tit">
          ...
          <div class="clm_acdo_tit_left">
            <a href="javascript:goPageOrderPageList();" class="btn b_prev"><span class="sp_prev">이전</span></a>
          </div>
        </div>
        <!--// 타이틀 --> 
        <!-- 1.주문정보 -->
        <div class="clm_acdo_tit2">
          <h2 class="h2_txo"> <strong><em class="num">1. </em></strong>주문정보</h2>
          <a href="javascript:void(0)" class="tit_desc"> <span class="sp_odr ic_arr">&nbsp;</span> </a>
        </div>
        <div class="clm_acdo_sec">
          <dl class="clm_ip2">
            <dt><span class="tit">주문일</span></dt>
            <dd>
						  ...
						  <c:choose>
								<c:when test="${asVO.receiveType=='02'}">
									<c:if test="${asVO.asState=='05'}">
									  <strong><font style="color:red">A/S대행요청</font></strong>▶센터주문완료▶택배수령요청▶A/S완료
									</c:if>
							        <c:if test="${asVO.asState=='06'}">
									     A/S대행요청▶<strong><font style="color:red">센터주문의뢰</font></strong>▶택배수령요청▶A/S완료
									</c:if>
									<c:if test="${asVO.asState=='08'}">
									     A/S대행요청▶센터주문의뢰▶<strong><font style="color:red">택배수령요청</font></strong>▶A/S완료
									</c:if>
									<c:if test="${asVO.asState=='09'}">
									     A/S대행요청▶센터주문의뢰▶택배수령요청▶<strong><font style="color:red">A/S완료</font></strong>
									</c:if> 
			            	 	</c:when>
							 <c:otherwise>
									<c:if test="${asVO.asState=='05'}">
									     <strong><font style="color:red">A/S대행요청</font></strong>▶센터주문의뢰▶매장수령요청▶A/S완료
									</c:if>
									<c:if test="${asVO.asState=='06'}">
									     A/S대행요청▶<strong><font style="color:red">센터주문의뢰</font></strong>▶매장수령요청▶A/S완료
									</c:if>
									<c:if test="${asVO.asState=='07'}">
									     A/S대행요청▶센터주문의뢰▶<strong><font style="color:red">매장수령요청</font></strong>▶A/S완료
									</c:if>
									<c:if test="${asVO.asState=='09'}">
									     A/S대행요청▶센터주문의뢰▶매장수령요청▶<strong><font style="color:red">A/S완료</font></strong>
									</c:if>
							 </c:otherwise>
			 			  </c:choose> 
            	 	</c:when>
